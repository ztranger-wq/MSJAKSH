...
import { FaUser, FaShoppingBag, FaHeart, FaMapMarkerAlt, FaCog, FaChartLine, FaEdit, FaCamera } from 'react-icons/fa';
...
const ProfilePage = () => {
  ...
  const handleImage=e=>{
    ...
    if(file){
      ...
      updateProfile(fd).then(()=>setMessage('Image updated!')).catch(()=>setError('Image upload failed'));
    }
  };
        ...
        <div className="overview-content">
          <div className="profile-header">
            <div className="profile-avatar">
              <img src={user.profileImage||'/api/placeholder/120/120'} alt="Profile" className="avatar-image"/>
              <label className="avatar-upload" htmlFor="profile-image"><FaCamera/>
                <input type="file" id="profile-image" accept="image/*" hidden onChange={handleImage}/>
              </label>
            </div>
            <div className="profile-info">
              <h2>{user.name}</h2><p className="email">{user.email}</p>
              <div className="membership-badge"><span className={`tier-badge tier-${(user.membershipTier||'bronze').toLowerCase()}`}>{user.membershipTier||'Bronze'} Member</span></div>
